// Deserialize and execute the FFT2D engine. An impulse at spatial index
// (0, 0) has a constant 2-D FFT, giving a reference without a CPU FFT.
// Usage:
//   run_fft2d_engine [model.engine] [plugin-library.so]
...
int main(int argc, char** argv)
{
    ...
    if (inputShape.nbDims < 2)
    {
        std::cerr << "FFT2D input must have at least two dimensions\n";
        return EXIT_FAILURE;
    }
    size_t const height{
        static_cast<size_t>(inputShape.d[inputShape.nbDims - 2])};
    size_t const width{
        static_cast<size_t>(inputShape.d[inputShape.nbDims - 1])};
    size_t const transformElements{height * width};
    size_t const elementCount{hostBuffers.at("real_input").size()};

    // For every HxW transform, x[0, 0] = 2 - 0.5j and other samples are zero.
    // Its unnormalized forward FFT is 2 - 0.5j at every frequency bin.
    for (size_t offset{0U}; offset < elementCount;
         offset += transformElements)
    {
    ...
    for (size_t i{0U}; i < elementCount; ++i)
    {
        if (std::abs(hostBuffers.at("real_output")[i] - 2.0F) > tolerance
            || std::abs(hostBuffers.at("imag_output")[i] + 0.5F)
                > tolerance)
        {
            std::cerr << "FFT2D verification failed at element " << i << ": "
    ...
    std::cout << "FFT2D end-to-end verification passed for "
              << elementCount / transformElements << " transforms of shape "
              << height << 'x' << width << ".\n";
    return EXIT_SUCCESS;
}
