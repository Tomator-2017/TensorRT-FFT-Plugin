// Build a TensorRT 8.6 engine from an ONNX graph containing the FFT2D plugin.
// Usage:
//   build_fft2d_engine [model.onnx] [model.engine] [plugin-library.so]
...
int main(int argc, char** argv)
{
    ...
    // Register FFT2DPluginCreator before the parser sees the custom node.
    ...
    {
        std::cerr << "FFT2D demo expects two FP32 inputs and outputs\n";
        return EXIT_FAILURE;
    }
    ...
    if (!serializedEngine)
    {
        std::cerr << "Failed to build FFT2D TensorRT engine\n";
        return EXIT_FAILURE;
    }
    ...
    std::cout << "Serialized FFT2D engine to " << enginePath << '\n';
    return EXIT_SUCCESS;
}
